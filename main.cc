...
class cOcean {
  ...
  public:
    ...
    void initTextures() {
        ...
        u_causticTexture = glGetUniformLocation(glProgram, "u_causticTexture");
		u_causticIntensity = glGetUniformLocation(glProgram, "u_causticIntensity");
    ...
    for (int y = 0; y < 64; y++) {
        for (int x = 0; x < 64; x++) {
            ...
            if (textureID == foamTexture) {
                ...
                data[idx+1] = 255;
                data[idx+2] = 255;
                data[idx+3] = static_cast<unsigned char>(noise * 200);
            }
            else {
                ...
                data[idx] = static_cast<unsigned char>((nx + 1.0f) * 127.5f);
                data[idx+1] = static_cast<unsigned char>((ny + 1.0f) * 127.5f);
                data[idx+2] = static_cast<unsigned char>((nz + 1.0f) * 127.5f);
                data[idx+3] = 255;
            }
        }
    }
...
void cOcean::render(float t, glm::vec3 light_pos, glm::mat4 Projection, glm::mat4 View, glm::mat4 Model, bool use_fft, bool underWater) {
    ...
    if (u_causticIntensity != -1) {
        glUniform1f(u_causticIntensity, 0.1f); 
    }
...
int main(int argc, char *argv[]) {
	...
	const float SURFACE_THRESHOLD = 25.0f, UNDER_THRESHOLD = 10.0f;
	...
	while(active) {
		...
		if (video_grab) {
			...
			ocean.render(elapsed_video, light_position, Projection, View, Model, use_fft, underWater);
			SDL_GL_SwapBuffers();
			buffer.save(true);
		} else {
			ocean.render(t1.elapsed(false), light_position, Projection, View, Model, use_fft, underWater);
			SDL_GL_SwapBuffers();
		}
